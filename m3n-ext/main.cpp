...
void test(M3N *m, istream &fin, ostream &fout, int margin, int nbest, int nbest_score){
	vector < char* >table;
	int total=0;
	int error=0;
	int index=0;
...
L0:
	...
	cerr << "Done!" << endl;
	cerr<<"label precision:"<<(1-(double)error/total)<<endl;
}

void score(M3N *m, istream &fin, istream &fnbest, ostream &fout, int mode){
	vector <char*> table;
	vector <vector <char*> > lab_seqs;
	int total=0;
	int error=0;

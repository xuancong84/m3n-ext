...
void forward_backward_viterbi(int ysize,
							  ...
							  //vector<int> &optimum_paths,
							  vector<int> &alpha_path,
							  vector<int> &beta_path,
							  double head_off=0,
...
template <class T>
void find_top2(const vector<T> &v, vector<int> &index)
{// find top 2 indices with the best corresponding v; index must have a size >=2
	double top1=-INF, top2=-INF;
	for(int x=0, X=v.size(); x<X; ++x){
		double val=v[x];
		if(val>top2){
			if(val>top1){	// if better than the best
				index[1]=index[0];
				index[0]=x;
				top2=top1;
				top1=val;
			}else{			// if better than the 2nd best
				index[1]=x;
				top2=val;
			}
		}
